                        ...
                        <table class="table table-striped" id="dataTable">
                            ...
                            <tbody>
                                @foreach ($booking as $index => $booking)
                                    <tr>
                                        <td>{{ ++$index }}</td>
                                        <td><a href="#">{{ $booking->property_id }}</a></td>
                                        <td>{{ $booking->name }}</td>
                                        <td>
                                        ...
                                        <td>
                                            @if($booking->status == 1)
                                            <a href="javascript:;" onclick="changeBlogStatus({{ $booking->id }})">
                                                <input id="status_toggle" type="checkbox" checked data-toggle="toggle" data-on="{{__('admin.Confirmed')}}" data-off="{{__('admin.Pending')}}" data-onstyle="success" data-offstyle="danger">
                                            </a>

                                            @else
                                            <a href="javascript:;" onclick="changeBlogStatus({{ $booking->id }})">
                                                <input id="status_toggle" type="checkbox" data-toggle="toggle" data-on="{{__('admin.Confirmed')}}" data-off="{{__('admin.Pending')}}" data-onstyle="success" data-offstyle="danger">
                                            </a>
