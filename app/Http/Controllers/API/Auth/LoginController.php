...
class LoginController extends Controller
{
    ...
    public function storeLogin(Request $request){
        ...
        $user = User::where('email',$request->email)->select('id','name','email','phone','user_name','status','password','image','address','designation','about_me','facebook','twitter','linkedin','instagram')->first();
        if($user){
            if($user->status==1){
